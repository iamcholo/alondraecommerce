define(['angular','clipboard'],function(angular,clipboard){
 	angular.module('app.controllers.custom.games', [])
 	.controller('GamesListCtrl', 
	[ '$scope','$state','$translate','Games',
	  function ($scope,$state,$translate, Games) 
	  {
		...
		$scope.makeTodos = function()
		{
			$scope.todos = [];
			$scope.filteredTodos = [];
		    Games.list().then(function successCallback(response)
		    {
		...
		{
			Games.Delete(id).then(function successCallback(response){
				$scope.makeTodos(); 
			}, function errorCallback(response) {});
		}
	...
	[ '$scope','$state','$translate','$stateParams','Games','Tags','Category','Media',
	  function ($scope,$state,$translate,$stateParams,Games,Tags,Category,Media) 
	  {
		...
		$scope.name = '';
	  	...
	  	Games.Get( $stateParams.id ).then(function successCallback(response){
	  ...
	  function ($scope,$state,$translate,$stateParams,Games,Tags,Category,Media) 
	  {
		...
		$scope.query = '';
	  	...
	  	$scope.Save = function()
	  	{
	  		...
	  		Games.New($scope.model).then(function successCallback(response)
		    {
